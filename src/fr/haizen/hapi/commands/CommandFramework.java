...
import fr.haizen.hapi.commands.annotations.Completer;
...
public class CommandFramework implements CommandExecutor {
		...
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if ((!command.isConsole()) && (!(sender instanceof Player))) {
					sender.sendMessage("Cette commande est uniquement disponible en jeu !");
					return true;
				}
	...
	private void defaultCommand(CommandArgs args) {
		args.getSender().sendMessage(Utils.color("§eDésolé, la commande §6" + args.getLabel() + " §en'existe pas."));
	}
}
